...
/**
 * Shared listing for /programs and /workshops (Figma #1582:16181 and
 * #1988:7449 — identical layouts, only the nav underline differs).
...
export function DiscoveryListing({
  ...
  const variantCourses = useMemo(
    () =>
      courses.filter((c) =>
        variant === "programs" ? c.course_type !== "short_term" && c.course_type !== "summer" : c.course_type === "short_term" || c.course_type === "summer"
      ),
    [courses, variant]
  )
  ...
  const filtered = useMemo(() => {
    ...
    return variantCourses.filter((course) => {
      ...
      if (q) {
        const haystack = [course.name, course.intro, course.instructor, course.program_code]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
    ...
    <div className="flex flex-col lg:mx-auto lg:max-w-[1440px]">
      ...
      {hasNoWorkshops ? (
        <div className="flex flex-col items-center gap-4 px-6 py-24 text-center">
          <p className="text-lg text-shade-500">{t("programs.workshopsEmpty")}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-8 px-6 pb-16 lg:flex-row lg:gap-0 lg:px-0 lg:pb-0">
          ...
          <FilterSidebar selectedDistricts={districts} onToggleDistrict={toggleDistrict} />
          ...
          <div className="min-w-0 lg:basis-[1102.2px] lg:grow-0">
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center gap-4 py-24 text-center">
                <p className="text-base font-semibold text-ink">{t("programs.emptyTitle")}</p>
                <p className="text-sm text-shade-500">{t("programs.emptyBody")}</p>
                <button
                  type="button"
