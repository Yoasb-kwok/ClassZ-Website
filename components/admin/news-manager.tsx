...
import { RichTextEditor } from "@/components/admin/page-editor/rich-text-editor";
import {
  AdminCard,
  AdminInput,
  AdminLabel,
...
export function NewsManager() {
    ...
    if (editing) {
      patch((s) =>
        appendAudit(
          {
            ...s,
            newsPosts: s.newsPosts.map((x) => (x.id === editing.id ? post : x)),
          },
          {
            action: "update_news",
            target_type: "news",
            target_id: post.id,
            details: post.title_zh_tw,
          },
        ),
      );
    } else {
  ...
  function remove(id: string) {
    ...
    patch((s) =>
      appendAudit(
        { ...s, newsPosts: s.newsPosts.filter((x) => x.id !== id) },
        {
          action: "delete_news",
          target_type: "news",
          target_id: id,
          ...
          {store.newsPosts.map((p) => (
            ...
            >
              <div>
                ...
                <p className="text-base text-classz-500 mt-1">
                  {p.published_at}
                </p>
              </div>
              <div className="flex gap-2">
                <button
                  type="button"
                  className="p-2 text-classz-500 hover:bg-classz-50 rounded"
                  onClick={() => openEdit(p)}
                >
                ...
                <button
                  type="button"
                  className="p-2 text-brand-coral hover:bg-[color-mix(in_srgb,var(--brand-coral)_10%,white)] rounded"
                  onClick={() => remove(p.id)}
                >
      ...
      <AdminModal
        ...
        footer={
          <>
            <button
              type="button"
              className="px-4 py-2.5 text-base text-classz-700 border border-classz-200 rounded-md hover:bg-classz-50"
        ...
        <div className="space-y-3">
          <div>
            ...
            <AdminInput
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, title_zh_tw: e.target.value }))
              }
            />
          </div>
          <div>
          ...
          <label className="flex items-center gap-2 text-base text-classz-700">
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, show_as_popup: e.target.checked }))
              }
            />
            {zh ? "首頁彈窗" : "Show as popup"}
          </label>
          <div>
            <AdminLabel>Content (ZH)</AdminLabel>
            <RichTextEditor
              value={form.content_zh_tw}
              onChange={(html) =>
                setForm((f) => ({ ...f, content_zh_tw: html }))
              }
            />
          </div>
          <div>
            <AdminLabel>Content (EN)</AdminLabel>
            <RichTextEditor
