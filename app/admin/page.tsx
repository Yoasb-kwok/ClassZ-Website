    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md border border-classz-200 border-l-4 border-l-classz-400 p-5">
        ...
        {todayClasses.length > 0 && (
          <div className="mt-4 pt-4 border-t border-classz-200">
            <p className="text-sm font-semibold text-classz-600 uppercase mb-2">{t("classzAdmin.todayClassList")}</p>
            <ul className="space-y-2">
              {todayClasses.map((c) => (
                <li key={c.id}>
                  <Link
                    href="/admin/courses"
                    className="flex items-center justify-between py-2 px-3 rounded-md hover:bg-classz-50 text-classz-800"
                  >
                    <span className="font-medium">{c.name}</span>
                    <span className="text-base text-classz-600">{formatClassTime(c.start_time, locale)}</span>
                    <ChevronRight className="h-4 w-4 text-classz-400" />
                  </Link>
                </li>
              ))}
            </ul>
        ...
        {filtered.length === 0 ? (
          ...
          <ul className="space-y-3">
            {filtered.map((c) => (
              <li
                key={c.id}
                className="flex justify-between items-center p-4 bg-classz-50/80 rounded-lg border border-classz-100"
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-classz-800">{c.name}</span>
                    {c.program_code && (
                      <span className="text-sm px-2.5 py-1 bg-classz-100 text-classz-700 rounded font-medium border border-classz-200">
                        {c.program_code}
                      </span>
                    )}
                  </div>
                  ...
                  {c.location && <div className="text-sm text-classz-500 mt-1">{locationLabel(c.location)}</div>}
                </div>
                <div className="text-right">
                  <div className="text-base font-medium text-classz-800">
                    {c.enrolled_count} / {c.capacity}
                  </div>
                  <div className="text-sm text-classz-500">{t("classzAdmin.enrolled")}</div>
                </div>
              </li>
            ))}
          </ul>
