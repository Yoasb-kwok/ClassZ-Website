...
| A11 | **HTML fields are edited with a WYSIWYG, not raw HTML**: all `body_html` / `answer_html` fields (about/terms/privacy bodies, FAQ answers) and the news content fields render through a TipTap-based `RichTextEditor` (bold/italic/h3/h4/lists/link/image via `/api/admin/uploads`, clear-formatting, char count, collapsed "View HTML" disclosure). **Storage stays HTML** — no data change; the toolbar emits exactly the markup the API validates. TipTap v2 (MIT) pinned. | User decision 2026-09-17 — "admins should not edit HTML directly" |
